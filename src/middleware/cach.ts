...
const cachedRoute = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if(cached){
        res.sendFile(path.join(root, `../resizedImage/${imageName}-${width}-${height}.JPG`))
    }
    else{
        next()
    }
